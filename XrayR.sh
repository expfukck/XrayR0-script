...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/straint/XrayR-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/straint/XrayR-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/straint/XrayR-script ---
  ${green}0.${plain} 修改配置
————————————————
